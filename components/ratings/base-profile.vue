<template>
    <main class="c_home">
        ...
        <div class="p-4 m-auto">
            <AppText variant="16" font="600">{{ pageTitle }}<br />
            </AppText>

            <div class="w-full flex my-3 py-3">
                <div class="my-2">
                    <img width="70px" :src="candidate?.image" alt="dummy" />
                </div>
                ...
                <template v-slot:profile>
                    <div class="c_political">
                        ...
                        <div v-for="item in political_info" v-bind:key="item.title" class="c_political-item">
                            ...
                            <AppText variant="16" font="300" class="right text-right float-right text-wrap">{{ typeof candidate?.[item.title.toLowerCase()] === 'object' ? `${candidate.party.name} (${candidate.party.slug})` :  candidate?.[item.title.toLowerCase()]  }}
                            </AppText>
                        </div>
                    </div>
                </template>
            ...
            <nuxt-link class="submit" :to="`/ratings/vote?candidate_id=${candidate?.candidateId}`">
                Submit Feedback
            </nuxt-link>
...
export default {
    name: "BaseProfile",
    middleware: "index",
    components: { AppText, post: Post, Tabs, poll: Poll, spinner: Spinner, PerformanceRating },
    props: {
        pageTitle: {
            type: String,
            default: ''

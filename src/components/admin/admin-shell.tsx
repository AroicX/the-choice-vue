      ...
      {adminNav.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium text-slate-300 transition-colors hover:bg-white/10 hover:text-white",
    ...
    <div className="h-screen overflow-hidden bg-slate-100 text-slate-950">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-800 bg-slate-950 text-white lg:flex lg:flex-col">
        <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
          <img src="/legacy/logo.png" alt="Choice9ja" className="h-10 w-10 rounded-xl bg-white object-contain" />
          <div>
        ...
        <div className="border-t border-white/10 p-4">
          <p className="truncate text-sm font-medium text-white">{user?.firstName ?? "Admin"} {user?.lastName ?? ""}</p>
      ...
      {mobileNavOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          <button type="button" className="absolute inset-0 bg-slate-950/50 backdrop-blur-sm" aria-label="Close menu" onClick={() => setMobileNavOpen(false)} />
          <aside className="absolute inset-y-0 left-0 flex w-72 flex-col bg-slate-950 text-white shadow-2xl">
            <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
              <div className="flex items-center gap-3">
                <img src="/legacy/logo.png" alt="Choice9ja" className="h-9 w-9 rounded-xl bg-white object-contain" />
                <span className="text-sm font-semibold">Control</span>
              </div>
              <Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={() => setMobileNavOpen(false)}>
      ...
      <div className="flex h-screen flex-col lg:pl-72">
        <header className="z-20 flex h-16 shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/90 px-4 shadow-sm backdrop-blur-xl lg:gap-4 lg:px-6">
          ...
          <div className="relative hidden w-full max-w-sm md:block">
            <AppIcon icon={Search01Icon} size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <Input className="h-10 rounded-xl border-slate-200 bg-slate-50 pl-10" placeholder="Search control panel..." />
          </div>
          <Button variant="ghost" size="icon" className="rounded-xl text-slate-600" title="Notifications" asChild>
          ...
          <div className="relative">
            <button
              type="button"
              className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-left text-sm shadow-sm"
            ...
            {profileOpen ? (
              <div className="absolute right-0 mt-2 w-56 rounded-xl border bg-white p-2 shadow-panel">
                <Link href="/control/settings" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-slate-100" onClick={() => setProfileOpen(false)}>
                  <AppIcon icon={Settings01Icon} size={16} />
                  Settings
                </Link>
                <button type="button" className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50" onClick={handleLogout}>
